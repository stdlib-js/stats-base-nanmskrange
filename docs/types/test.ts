...
// The function returns a number...
{
	...
	nanmskrange( x.length, new AccessorArray( x ), 1, mask, 1 ); // $ExpectType number
}
...
// Attached to main export is an `ndarray` method which returns a number...
{
	...
	nanmskrange.ndarray( x.length, new AccessorArray( x ), 1, 0, new AccessorArray( mask ), 1, 0 ); // $ExpectType number
}
